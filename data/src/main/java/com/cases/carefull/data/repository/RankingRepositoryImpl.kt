...
class RankingRepositoryImpl() : RankingRepository {
	...
	override suspend fun getRankingList(sport: ExerciseType): DataResult<List<Ranker>> =
		runCatching {
			...
			DataResult.Success(rankerList)
			
		}.getOrElse { exception ->
			DataResult.Error(exception)
		}
	
	override suspend fun getMyRanking(userId: String, sport: ExerciseType): DataResult<MyRankInfo> =
		runCatching {
			val myRanking =
				db.collection("work_out_collection")
					.whereEqualTo("category_id", sport.name)
