...
class DumbbellCurlAnalyzer(private val isLeftHand: Boolean) : ExerciseAnalyzer {
	override fun analyze(pose: Pose): ExerciseState {
		val shoulder = pose.landmarks[if(isLeftHand) PoseLandmark.LEFT_SHOULDER else PoseLandmark.RIGHT_SHOULDER]
		val elbow = pose.landmarks[if(isLeftHand) PoseLandmark.LEFT_ELBOW else PoseLandmark.RIGHT_ELBOW]
		val wrist = pose.landmarks[if(isLeftHand) PoseLandmark.LEFT_WRIST else PoseLandmark.RIGHT_WRIST]
